...
const getTotalSales = (startDate, endDate) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const allProductSales = salesData.length > 0 ? salesData[0].productSales.flat() : [];
            ...
            if (mostOrderedProductId) {
                mostOrderedProduct = await productModel.findById(mostOrderedProductId).lean();
                if (mostOrderedProduct && mostOrderedProduct.category) {
                    const category = await categoryModel.findById(mostOrderedProduct.category).lean();
                    mostOrderedProduct.category = category ? category.name : "Unknown Category";
                }
            }
            ...
            resolve({
                totalSalesAmount: salesData.length > 0 ? salesData[0].totalSalesAmount : 0,
                totalNumberOfOrders: salesData.length > 0 ? salesData[0].totalNumberOfOrders : 0,
                totalQuantityOfProductsOrdered: salesData.length > 0 ? salesData[0].totalQuantityOfProductsOrdered : 0,
                totalRevenueFromDeliveredOrders: deliveredOrders.length > 0 ? deliveredOrders[0].totalRevenueFromDeliveredOrders : 0,
                orderBreakdownByStatus: salesData.length > 0 ? salesData[0].orderBreakdownByStatus : [],
...
const getMonthlySalesReport = (startDate, endDate) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const deliveredOrdersData = await orderModel.aggregate(deliveredOrdersAggregation);
            ...
            const formattedSalesData = monthlySalesData.map(data => {
                ...
                return {
                    ...
                    totalRevenueFromDeliveredOrders: deliveredRevenueMap.get(monthKey) || 0
                };
            });
